<?php $title = "About Us - GirlsTrade";  include("header.php"); ?>

<div id="wrapper">
  
  <!-- /.header -->
  
  <div class="intro-inner">
    <div class="about-intro" style="
    background:url(../images/about-us-bg.jpg) no-repeat center;
	background-size:cover;">
      <div class="dtable hw100">
        <div class="dtable-cell hw100">
          <div class="container text-center">
            <h1 class="intro-title animated fadeInDown"> Building a customer focus </h1>
          </div>
        </div>
      </div>
    </div>
    <!--/.about-intro --> 
    
  </div>
  <!-- /.intro-inner -->
  
  <div class="main-container inner-page">
    <div class="container">
      <div class="section-content">
        <div class="row ">
          <h1 class="text-center title-1"> What Makes Us Special </h1>
          <hr class="center-block small text-hr">
          <div class="col-sm-6">
            <div class="text-content has-lead-para text-left">
              <p class="lead"> 
              The word “Girlstrade” (pronounced like Girl’s Trade) provides a platform for girls within Hong Kong to make trades.
Girlstrade offers a useful, enjoyable, and easy way for people within Hong Kong to buy, sell, and trade to support each other out in any girls related matters (such as goods, services, and jobs).
Bringing the right buyer and seller together is what we are trying to do within the website. Focusing solely on female goods and services makes Girlstrade the best e-platform for girls to gather and help each other out.
              </p>
            </div>
          </div>
          <div class="col-sm-6"> <img src="<?php echo base_url();?>images/info.png" alt="imfo" class="img-responsive"> </div>
        </div>
      </div>
    </div>
  </div>
  <!-- /.main-container -->
  <div class="parallaxbox about-parallax-bottom">
    <div class="container">
      <div class="row text-center featuredbox">
        <div class="col-sm-4 xs-gap">
          <div class="inner">
            <div class="icon-box-wrap"> <i class="icon-book-open ln-shadow-box shape-3"></i></div>
            <h3 class="title-4">Customer service</h3>
            <p>Girlstrade aims at providing the best service to our users. Connecting the right buyers and sellers is what we want to achieve.</p>
          </div>
        </div>
        <div class="col-sm-4 xs-gap">
          <div class="inner">
            <div class="icon-box-wrap"> <i class=" icon-lightbulb ln-shadow-box shape-6"></i></div>
            <h3 class="title-4">Seller satisfaction</h3>
            <p>Seller ratings are given by buyers according to their satisfaction about the trade. This information can on one hand facilitate buyers to make purchasing descision, while on the other hand help sellers to attract potential buyers.</p>
          </div>
        </div>
        <div class="col-sm-4 xs-gap">
          <div class="inner">
            <div class="icon-box-wrap"> <i class="icon-megaphone ln-shadow-box shape-5"></i></div>
            <h3 class="title-4">Best Offers </h3>
            <p>Girlstrade provide a platform designated for girls in Hong Kong to not only make trades but also gather and help each other out. </p>
          </div>
        </div>
      </div>
    </div>
  </div>
  <?php include "footer1.php"; ?>
  <!-- /.footer --> 

<!-- /.wrapper --> 
</div>
<?php include "footer2.php"; ?>
